(ns nmea2000_marine.main
  "Kotodama WASM entrypoint for the nmea2000_marine clean-room actor (L4) — Clojure port.
  ...
  facts (`nmea2000_marine.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nmea2000_marine")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nmea2000_marine.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nmea2000_marine-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nmea2000_marine-compat/ready)
