(ns nmea2000_marine.main-test
  "Contract + behavioral test for the nmea2000_marine-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "nmea2000_marine-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'nmea2000_marine.main-test)]
